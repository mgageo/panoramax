...
#
plotImg <- function(img, alpha = 255, maxpixels = 50000000, new = TRUE, png = FALSE, col = FALSE, breaks = FALSE) {
  graphics.off()
  if (is(img, "SpatRaster")) {
    ncols <- ncol(img)
    nrows <- nrow(img)
    carp("%sx%s nlayers: %s", ncols, nrows, nlyr(img))
    dev.new(width = ncols, height = nrows, units = "px", pointsize = 12)
    par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0), mai = c(0, 0, 0, 0))
    if (! is.logical(col)) {
      plot(img, axes = FALSE, add = FALSE, legend = FALSE, maxcell = maxpixels, col = col, breaks = breaks)
    } else {
      if (nlyr(img) < 3) {
        plot(img, axes = FALSE, add = FALSE, legend = FALSE, maxcell = maxpixels, box = FALSE)
      } else {
        plotRGB(img, axes = FALSE, add = FALSE, legend = FALSE, maxcell = maxpixels, box = FALSE)
      }
    }
    return(invisible())
  }
...
plotVect <- function(x, max = 1024) {
  graphics.off()
  old_par <- par(mar = c(0, 0, 0, 0), oma = c(0, 0, 0, 0), mai = c(0, 0, 0, 0))
  on.exit(par(old_par))
  bb <- st_bbox(x)
  largeur <- bb[3] - bb[1]
  hauteur <- bb[4] - bb[2]
  dev.new(width = largeur, height = hauteur, units = "px", pointsize = 12)
#  return(invisible())
  y <- st_as_sfc(bb, crs = st_crs(x))
  plot(y, col = NA, border = NA)
  pux <- par("usr")
  rect(pux[1], pux[3], pux[2], pux[4], border = NA, col = "grey")
}
#
# image en gris
